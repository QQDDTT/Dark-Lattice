...
- **外环/轨道 (circel1, circel2)**: 环绕核心的纤细光环或线框结构，用于在 WebGL 中赋予自转代码。注意拼写为 `circel`。
- **悬浮粒子 (group1, group2, group3)**: 散落在核心周围的小型节点（球体或正方体），代表数据点。分为三组以实现分层旋转动画。
- **几何复杂度**: 顶点数 (Vertices) 建议控制在 10,000 以内，以确保在移动端设备 and 低性能浏览器上的流畅运行。
...
## 7. 脚本驱动动画逻辑 (Animation Logic)

前端脚本 `scene-3d.js` 将根据以下命名查找对象并应用动态效果：

| 对象名称 | 动画类型 | 逻辑说明 |
| :--- | :--- | :--- |
| `core` | 交互随动 | 响应鼠标位置进行微妙的偏移与轴向偏转。 |
| `circel1` | 连续旋转 | 绕 Y 轴顺时针旋转，建立空间纵深感。 |
| `circel2` | 连续旋转 | 绕 Y 轴逆时针旋转，与 `circel1` 形成交错视觉。 |
| `group1` | 快速旋转 | 绕 Y 轴快速公转，模拟活跃的数据交换。 |
| `group2` | 慢速旋转 | 绕 Y 轴慢速公转，形成背景流动的节奏感。 |
| `group3` | 异轴旋转 | 绕 Z 轴旋转，打破单一轴向的单调感。 |
