...
## 8. 搜索引擎提交与验证 (Search Engine Submission)

### 8.1 Google Search Console (GSC) 验证
为了启用 GSC 监控：
1.  **HTML 文件验证（推荐）**：
    *   从 GSC 下载验证文件（如 `google123.html`）。
    *   将其放入项目的 `static/` 根目录下。
    *   部署后，访问 `https://yourdomain.com/google123.html` 确认可访问。
2.  **Meta 标签验证**：
    *   将 GSC 提供的 `<meta name="google-site-verification" content="..." />` 添加至 `layouts/partials/head.html`。

### 8.2 站点地图 (Sitemaps) 提交
*   **路径**：Hugo 会自动生成 `/sitemap.xml`。
*   **操作**：在 GSC 的 "Sitemaps" 模块中输入站点比例地址：`https://yourdomain.com/sitemap.xml`。
*   **更新频率**：每当新内容发布并部署至生产环境后，GSC 会自动检测 sitemap。

### 8.3 索引请求与监控
*   **URL 检查工具**：新发布的关键研究报告或文章，应手动在 GSC 中点击“请求编入索引”以加速收录。
*   **增强功能报告**：定期检查“核心网页指标”与“移动设备易用性”报告，修复潜在错误。

### 8.4 Robots.txt 指令
*   **文件位置**：`/static/robots.txt` 或由 Hugo 动态生成。
*   **基本规则**：
    ```text
    User-agent: *
    Allow: /
...
### 8.5 Bing Webmaster Tools
*   **快捷提交**：可直接使用 Google Search Console 账号导入站点设置，无需重复验证。
*   **IndexNow**：建议集成 IndexNow 协议以便在内容更新时瞬间通知 Bing 与 Yandex。
